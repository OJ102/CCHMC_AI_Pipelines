...
class Embedding:
    ...
    out_name (str): Path to the output file where the embeddings will be stored.
    level (str): Level of embedding ('protein', 'residue', or 'both').
    embed (str): The embedding model to use ('prott5', 'esm', 'esm1b', or 'onehot').
    """
    def __init__(self, in_file, out_name, out_dir=DATA_DIR, level='residue', embed='prott5'):
    ...
    def embedding(self):
        ...
        if not(l==0): #create two files for both option
            self.__file_initialize(f'./{self.out_dir}/{self.out_name}_residue_{self.embed}.csv',vect_len,l)
        if not(l==1):
            self.__file_initialize(f'./{self.out_dir}/{self.out_name}_protein_{self.embed}.csv',vect_len)
        # Running embeddings/writing to csv row by row
        return self.__embed_one_file(sequences=seq_dict, out_dir=self.out_dir, out_name=self.out_name, embed=self.embed,  is_residue=l)
    ...
    def __embed_one_file(self, sequences, out_dir, out_name, embed='prott5', is_residue=True):
        """
        ...
        out_name (str): Name of the CSV file.
        ...
        for protein in sequences:
            ...
            if not(is_residue==1):
                ...
                self.__write_csv(f'./{out_dir}/{out_name}_protein_{embed}.csv', reduced_embeddings)
            ...
            # residue level
                ...
                for idx, char in enumerate(sequences[protein]):
                    ...
                    self.__write_csv(f'./{out_dir}/{out_name}_residue_{embed}.csv', res_embed)
                    # res_embed.clear()
        return f'./{out_dir}/{out_name}_residue_{embed}.csv' if not(is_residue==0) else f'./{out_dir}/{out_name}_protein_{embed}.csv'
